...
export default function SmartChefLPEarnArchived({ category = 'lp' }: SmartChefLPEarnProps) {
      ...
      <AutoColumn gap="lg" style={{ width: '100%', maxWidth: '720px' }}>
        <RowBetween style={{ alignItems: 'baseline' }}>
          <TYPE.mediumHeader style={{ marginTop: '0.5rem' }}>
            {toTitleCase(category)} Pools - archived
          </TYPE.mediumHeader>
          <CombinedTVL />
        </RowBetween>
          ...
          {account && stakingRewardsExist && stakingPools?.length === 0 ? (
            ...
            poolsToDisplay
              ...
              .map(stakingInfo => {
                ...
                return (
                  <PoolCard key={stakingInfo.pid} category={category} stakingInfo={stakingInfo} isArchived={true} />
                )
              })
          )}
