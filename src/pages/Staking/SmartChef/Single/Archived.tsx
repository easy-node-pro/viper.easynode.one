...
export default function SmartChefSingleEarnArchived() {
  const { chainId, account } = useActiveWeb3React()
  const govToken = useGovernanceToken()
  const inactivePoolsOnly = false

  const category = 'single'
  const stakingPools = useSmartChefStakingPools(category, inactivePoolsOnly)
  ...
  const inactiveStakingPools = useFilterSmartChefStakingPools(stakingPools, inactivePoolsOnly)

  let poolsToDisplay = inactiveStakingPools
    ...
    <PageWrapper gap="lg" justify="center">
        ...
        <ExtraDataCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              ...
              <RowBetween>
                <StyledInternalLink to={`/staking/${category}`}>
      ...
      <AutoColumn gap="lg" style={{ width: '100%', maxWidth: '720px' }}>
        <RowBetween style={{ alignItems: 'baseline' }}>
          <TYPE.mediumHeader style={{ marginTop: '0.5rem' }}>{nestSettings?.name} - archived</TYPE.mediumHeader>
          <CombinedTVL />
        </RowBetween>
