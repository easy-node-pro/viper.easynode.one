...
import PoolCard from '../../../../components/Staking/SmartChef/Single/PoolCard'
...
import {
  ...
  CardNoise,
  CardBGImage
} from '../../../../components/Staking/SmartChef/Single/styled'
...
import { NEST_SETTINGS } from '../../../../constants'
import CombinedTVL from '../../../../components/Staking/TVL/Combined'
import { performAlternativeTokenSearch } from '../../../../utils/tokens/performTokenSearch'
...
export default function SmartChefSingleEarn() {
  ...
  const totalPoolCategoryCount =
    typeof chainId === 'number' && (SMART_CHEF_POOL_INFOS[chainId]?.[category]?.length ?? 0)
  const stakingRewardsExist = Boolean(totalPoolCategoryCount > 0)

  const activeStakingPools = useFilterSmartChefStakingPools(stakingPools, activePoolsOnly)
  ...
  let poolsToDisplay = activeStakingPools
  ...
  const handleInput = useCallback(event => {
    ...
    const searchedPools: any[] = performAlternativeTokenSearch(chainId, pools, 'stakedToken', 'rewardToken', input)
    ...
    <PageWrapper gap="lg" justify="center">
      <TopSection gap="md">
        <ExtraDataCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>{govToken?.name}Nest staking</TYPE.white>
              </RowBetween>
              <RowBetween>
      ...
      <AutoColumn gap="lg" style={{ width: '100%', maxWidth: '720px' }}>
        <RowBetween style={{ alignItems: 'baseline' }}>
          <TYPE.mediumHeader style={{ marginTop: '0.5rem' }}>{nestSettings?.name}</TYPE.mediumHeader>
          <CombinedTVL />
        </RowBetween>
        ...
        <PoolSection>
          {account && stakingRewardsExist && stakingPools?.length === 0 && totalPoolCategoryCount === 0 ? (
