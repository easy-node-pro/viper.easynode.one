...
export default function v1SwapArguments(
  ...
  const inputETH = trade.inputAmount.currency && DEFAULT_CURRENCIES.includes(trade.inputAmount.currency)
  const outputETH = trade.outputAmount.currency && DEFAULT_CURRENCIES.includes(trade.outputAmount.currency)
