...
export default function useCalculateWithdrawalFee(
  ...
  const userInfo = useSingleCallResult(masterBreeder, 'userInfo', [pid, account])?.result
  ...
  if (
    (!lastWithdrawBlock || lastWithdrawBlock.eq(BigNumber.from(0))) &&
    (!firstDepositBlock || firstDepositBlock.eq(BigNumber.from(0))) &&
    (!blockDelta || blockDelta.eq(BigNumber.from(0))) &&
    (!lastDepositBlock || lastDepositBlock.eq(BigNumber.from(0)))
  ) {
