...
export function usePitContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? PIT_TOKENS[chainId].address : undefined, PIT_ABI, withSignerIfPossible)
}

export function usePitBreederContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? PIT_BREEDER_ADDRESSES[chainId] : undefined, PIT_BREEDER_ABI, withSignerIfPossible)
}
...
export function useMasterBreederContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  const address = chainId && MASTER_BREEDER_ADDRESSES[chainId]
  return useContract(address, MASTER_BREEDER_ABI, withSignerIfPossible)
}

export function useSmartChefContract(address: string | undefined, withSignerIfPossible?: boolean): Contract | null {
  return useContract(address, SMART_CHEF_ABI, withSignerIfPossible)
}
